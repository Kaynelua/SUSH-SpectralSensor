...
def write(bus, vaddr:int, val:int):
	while(bus.read_byte_data(0x49,0x00) & 0x02):		# check tx_valid bit
		pass
...
def read(bus, vaddr:int,):
	...
	while(not bus.read_byte_data(0x49,0x00) & 0x01):		# check rx_valid bit
		pass
	return bus.read_byte_data(0x49,0x02) 		# read data from buffer
